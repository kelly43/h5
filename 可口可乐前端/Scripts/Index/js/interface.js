/**
 * Created by Administrator on 2016/8/5 0014.
 */
function IHotyi(){}
IHotyi.Config = {
    //是否启用json demo 调试
    IS_JSON_DEMO_TEST: true,
    JSON_DEMO: {
        //1.用戶基本信息
        UserRecord: {
            "Code": 1,
            "msg": "sucessful",
            "Time": "/Date(1474559336740)/",
            "Data": {
                "Id": 1,
                "IsFirst": true,
                "DrawNum": 2,
                "IsQuestions": false,
                "CustomsIds": "1",
                "AwardRecord": [
                    {
                        "Id": 4,
                        "AwardName": "海洋王國萬聖節夜間門票8折",
                        "AwardDesc": "",
                        "AwardType": 3,
                        "TermsConditions": "海洋王國萬聖節夜間門票8折海洋王國萬聖節夜間門票8折海洋王國萬聖節夜間門票" +
                        "8折海洋王國萬聖節夜間門票8折海洋王國萬聖節夜間門票8折海洋王國萬聖節夜間門票8折",
                        "ApplyShop": "海洋王國萬聖節夜間門票8折海洋王國萬聖節夜間門票8折海洋王國萬聖節夜" +
                        "間門票8折海洋王國萬聖節夜間門票8折",
                        "AwardImg": "../../Content/Images/img/g3-4.png",
                        "ExchangeImg": "",
                        "ChanceKey": "PAFIIFBJ1",
                        "IsChange": false,
                        "UserName": "",
                        "UserPhoneCode": "",
                        "UserPhone": "",
                        "UserAddress": "",
                        "IsGet": true,
                        "ShopPhone": "(853)2823 4567",
                        "Eights": "45646456"
                    }
                ]
            }
        },
        //2.查找奖品信息
        CouponRecord: {
            "Code": 1,
            "msg": "sucessful",
            "Time": "/Date(1474559676865)/",
            "Data": {
                "Id": 6,
                "AwardName": "海洋王國萬聖節夜間門票8折",
                "AwardDesc": "",
                "AwardType": 2,
                "TermsConditions": "",
                "ApplyShop": "",
                "AwardImg": "",
                "ExchangeImg": "",
                "ChanceKey": "PAFIIFBJ1",
                "IsChange": false,
                "UserName": "",
                "UserPhoneCode": "",
                "UserPhone": "",
                "UserAddress": "",
                "IsGet": true,
                "IsWin": false
            }
        },
        //3.核销
        ExchangeCoupon: {
            "Code": 1,
            "msg": "done",
            "Time": "/Date(1474560299163)/",
            "Data": {
                "Statue": true,
                "StatueCode": 1
            }
        },
        //4.保存用户信息
        SaveInfo: {
            "Code": 1,
            "msg": "done",
            "Time": "/Date(1474560299163)/",
            "Data": {
                "Statue": true,
                "StatueCode": 1
            }
        },
        //5.抽獎
        GetDraw: {
            "Code": 1,
            "msg": "sucessful",
            "Time": "/Date(1474558982200)/",
            "Data": {
                "Id": 1,
                "AwardName": "海洋王國萬聖節夜間門票8折",
                "AwardDesc": "",
                "AwardType": 2,
                "TermsConditions": "海洋王國萬聖節夜間門票8折海洋王國萬聖節夜間門票8折海洋王國萬聖節夜間門票" +
                "8折海洋王國萬聖節夜間門票8折海洋王國萬聖節夜間門票8折海洋王國萬聖節夜間門票8折",
                "ApplyShop": "海洋王國萬聖節夜間門票8折海洋王國萬聖節夜間門票8折海洋王國萬聖節夜" +
                "間門票8折海洋王國萬聖節夜間門票8折",
                "AwardImg": "../../Content/Images/img/g3-1.png",
                "ExchangeImg": "",
                "ChanceKey": "PAFIIFBJ1",
                "IsChange": false,
                "UserName": "",
                "UserPhoneCode": "",
                "UserPhone": "",
                "UserAddress": "",
                "IsGet": true,
                "ShopPhone": "(853)2823 4567",
                "Eights": "45646456",
                "IsWin": true
            }
        },
        //6.系統狀態
        GameStatue: {
            "Code": 1, //状态（1、成功，2、失败）
            "msg": "",//状态描述
            "Time": "/Date(1476170003319)/",
            "Data": {
                "SystemStatue": 1//系统状态（1、正在进行，2、未开始，3、核销时间（不能留资、不能兑换），4、正式结束，0、错误）
            }
        }
    }
};
//接口调用无需判断Code==1哦
IHotyi.prototype={
    post:function(name,o,o2,call){
        if(IHotyi.Config.IS_JSON_DEMO_TEST){
            var jsonObj = IHotyi.Config.JSON_DEMO[name];
            if(jsonObj.Code==1){
                call(jsonObj);
            }else if(jsonObj.Code==3){
                window.location.href = window.location.href;
            } else {
                window.location.href = "/index/error";
            }
        }else{
            $.post(o, o2, function (data) {
                var jsonObj = JSON.parse(data);
                if(jsonObj.Code==1){
                    call(jsonObj);
                }else if(jsonObj.Code==3){
                    window.location.href = window.location.href;
                } else {
                    window.location.href = "/index/error";
                }
            });
        }
    },
    //这部分修改后 记得穿传进去的第一个参数是 接口名字哦

    iGameStatue:function(call){
        this.post("GameStatue","/Index/GameStatue",{},call)
    },
    iCouponRecord:function(call,obj){
        this.post("CouponRecord","/Index/CouponRecord",obj,call)
    },
    iExchangeCoupon:function(call,obj){
        this.post("ExchangeCoupon","/Index/ExchangeCoupon",obj,call)
    },
    iGetDraw:function(call){
        this.post("GetDraw","/Index/GetDraw",{},call)
    },
    iSaveInfo:function(call,obj){
        this.post("SaveInfo","/Index/SaveInfo",obj,call)
    },
    iUserRecord:function(call){
        this.post("UserRecord","/Index/UserRecord",{},call)
    }
}