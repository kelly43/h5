...
HotyiLoad.DiyLoad = {
    ...
    _loading: function (stage_main) {
        ...
        KWhaleUtils.getImgBySrcAndCall(that.logo_res, function (logo) {
            ...
            that.logo_txt_res = "../../Content/Images/img/logo_txt.png";
            KWhaleUtils.getImgBySrcAndCall(that.logo_txt_res, function (logo_txt) {
                that.logo_txt = new Bitmap(logo_txt);
                that.logo_txt.anchorX = that.logo_txt.anchorY = 0.5;
                KWhaleUtils.AddStageByScaleHNoCheck(stage_main, that.logo_txt, 30 / 1135);
                that.logo_txt.x = start_x;
                that.logo_txt.y = stage_main.sh / 1135 * 639;
            });
    ...
    _removeLoadingAnimate: function (call,stage_main) {
        ...
        var t1 = KWhaleUtils.Tween(that.logo_txt, {alpha: 0}, 0, 500, TWEEN.Easing.Quadratic.InOut).start();
        var t2 = KWhaleUtils.Tween(that.circle_wrap, {alpha: 0}, 0, 500, TWEEN.Easing.Quadratic.InOut).start().onComplete(function () {
            ...
            stage_main.removeChild(that.logo_txt);
